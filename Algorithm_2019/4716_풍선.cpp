...
int main() {
	...
	while (~scanf("%d %d %d", &n, &a, &b)) {
		...
		mcmf.add_edge(0, 1, a, 0);
		mcmf.add_edge(0, 2, b, 0);
		for (int i = 0; i < n; ++i) {
			int num, da, db; scanf("%d %d %d", &num, &da, &db);
			mcmf.add_edge(1, 3 + i, 1e9, da);
			mcmf.add_edge(2, 3 + i, 1e9, db);
			mcmf.add_edge(3 + i, 3 + n, num, 0);
		}
		printf("%d\n", mcmf.match(0, 3 + n));
		mcmf.clear();
	}
